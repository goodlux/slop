...
created: 2026-01-17T09:01:09.876199
...
User writes markdown
  ...
  → RDF graph built locally
  → Optional: sync shape to server for discovery
  → SPARQL queries find semantic matches
```
...
1. **GitHub App watches repos** → generates RDF triples on push
...
├─ code (GitHub)
└─ Solid Pod (repolex.myorg.ai)
   ├─ WebID (repo identity)
   ├─ RDF Graph (semantic code structure)
   ├─ Type Index (capabilities: "I can analyze security")
   ├─ LDN Inbox (notifications from other repos)
   └─ WebACL (fine-grained access control)
```
...
And it all started by accident, building a simple markdown renderer with entity highlighting. 🌊✨